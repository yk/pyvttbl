...
def interaction_plot(df, val, xaxis, 
    ...
    if xaxis not in list(df.keys()):
        raise KeyError(xaxis)
    
    if seplines not in list(df.keys()) and seplines != None:
        raise KeyError(seplines)

    if sepxplots not in list(df.keys()) and sepxplots != None:
        raise KeyError(sepxplots)
    
    if sepyplots not in list(df.keys()) and sepyplots != None:
    ...
    # check cell counts
    cols = [f for f in [seplines, sepxplots, sepyplots] if f in list(df.keys())]
